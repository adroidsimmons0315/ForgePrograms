...
def run_scan_in(root: tk.Misc | None = None):
    """
    Scan items IN to inventory.

    If found: update Z (Last Scanned), AA (Last Verified), X (In/Out) = "In", and I (Loc).
    If not found or duplicate: append a new row with seq (50xxx/51xxx),
    Asset Id in column C, I = chosen location, X = "In", Z/AA = today.
    """
    ...
    # Select Excel file
    data_path = sf.select_inventory_excel_file(root=root, title="Select Inventory Excel File (Scan IN)")
    if not data_path:
        if owns_root:
            root.destroy()
        return
    ...
    # Build list of existing locations from column I, excluding those starting with "SHR:"
    loc_values: set[str] = set()
    for r in range(2, ws.max_row + 1):
        val = ws[f"I{r}"].value
        if isinstance(val, str):
            v = val.strip()
            if v and not v.startswith("SHR:"):
                loc_values.add(v)
    ...
    ttk.Label(loc_row, text="Location:", font=("Segoe UI", 10, "bold")).pack(
        side="left", padx=(0, 8)
    )
    ...
    loc_combo.set(existing_locations[0] if existing_locations else "")
    # Don't steal focus from the scan entry; just prefill if you like.

    ttk.Label(
        container,
        text="Scan or type an asset code, then press Enter. "
             "Type 'done' to finish (Scan IN).",
        font=("Segoe UI", 11),
        wraplength=680,
        justify="left",
    ...
    def save_and_close():
        ...
        log("\n--- Scan IN Completed ---")
        log(f"Location used: {loc or '(none)'}")
    ...
    def handle_scan(event=None):
        ...
        if not loc:
            messagebox.showwarning(
                "Location required",
                "Please select or type a location before scanning.",
                parent=scan_win,
            )
            return
        ...
        if normalized_code in normalized_asset_set:
            ...
            if normalized_code in scanned:
                # DUPLICATE: append at bottom with 51xxx sequence, X="In", I=loc
                ...
                ws[f"X{row_num}"].value = "In"
                ws[f"Z{row_num}"].value = today
                ws[f"AA{row_num}"].value = today

                log(
                    f"[!] {code} duplicate ID. "
                    f"Added IN row {row_num} with A={seq}, C={code}, I='{loc}', X=In."
                )
            else:
                ...
                log(
                    ...
                    f"Set I{row_num}='{loc}', X{row_num}=In, "
                    f"Z{row_num}/AA{row_num}={today}."
                )
        else:
            # NOT FOUND: add new IN row with 50xxx sequence
            row_num = ws.max_row + 1
            seq = next_new_seq
            next_new_seq += 1
            new_items_count += 1
            ...
            log(
                ...
                f"Added IN row {row_num} with A={seq}, C={code}, I='{loc}', "
                f"X=In, Z/AA={today}."
            )
